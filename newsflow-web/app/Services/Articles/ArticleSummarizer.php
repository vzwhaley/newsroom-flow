...
class ArticleSummarizer
{
    ...
    {
        return (bool) config('newsflow.llm.enabled') && (bool) config('newsflow.llm.api_key');
    }
    ...
    private function fetchReadableText(string $url): string
    {
        try {
            $response = Http::withHeaders([
                'User-Agent' => 'Mozilla/5.0 (compatible; NewsroomFlowBot/1.0; +https://newsflow.test)',
                'Accept'     => 'text/html',
            ])->timeout(12)->get($url);
    ...
    private function askClaude(string $headline, string $source): ?string
    {
        try {
            ...
            ])->timeout(30)->post(config('newsflow.llm.endpoint'), [
