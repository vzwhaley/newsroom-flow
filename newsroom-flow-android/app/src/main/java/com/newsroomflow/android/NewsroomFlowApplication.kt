...
class NewsroomFlowApplication : Application() {
    ...
    private fun createNotificationChannel() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            ).apply { description = "Your daily NewsroomFlow summary and watchlist alerts." }
            getSystemService(NotificationManager::class.java)?.createNotificationChannel(channel)
        }
    }

    companion object {
        const val PUSH_CHANNEL_ID = "newsroomflow_default"
    }
}
