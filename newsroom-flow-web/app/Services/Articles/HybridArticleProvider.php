...
class HybridArticleProvider implements ArticleProvider, LocationAwareProvider
{
    ...
    {
        $pool = (int) config('newsroomflow.candidate_pool', 40);
    ...
    */
    ...
    private function gatherFromSources(string $topic, int $pool): array
    {
        ...
        if (config('newsroomflow.sources.google_news.enabled')) {
            $all = array_merge($all, $this->fromGoogleNewsRss($topic, $pool));
        }

        if ($key = config('newsroomflow.sources.thenewsapi.key')) {
            $all = array_merge($all, $this->fromTheNewsApi($topic, $pool, $key));
        }

        if ($key = config('newsroomflow.sources.newsdata.key')) {
            $all = array_merge($all, $this->fromNewsData($topic, $key));
        }

        if ($key = config('newsroomflow.sources.gnews.key')) {
            $all = array_merge($all, $this->fromGNews($topic, $pool, $key));
        }

        if ($key = config('newsroomflow.sources.newsapi.key')) {
    ...
    {
        return (bool) (config('newsroomflow.sources.google_news.enabled')
            || config('newsroomflow.sources.thenewsapi.key')
            || config('newsroomflow.sources.newsdata.key')
            || config('newsroomflow.sources.gnews.key')
            || config('newsroomflow.sources.newsapi.key'));
    }
    ...
    private function fromGoogleNewsRss(string $topic, int $pool): array
    {
        try {
            ...
            $response = Http::timeout(15)
                ...
                ->get(config('newsroomflow.sources.google_news.endpoint'), $params);
    ...
    private function fromTheNewsApi(string $topic, int $pool, string $key): array
    {
        try {
            ...
            $response = Http::timeout(15)->get(config('newsroomflow.sources.thenewsapi.endpoint'), $params);
    ...
    private function fromNewsApi(string $topic, int $pool, string $key): array
    {
        try {
            ...
            $response = Http::timeout(15)->get(config('newsroomflow.sources.newsapi.endpoint'), $params);
    ...
    private function fromGNews(string $topic, int $pool, string $key): array
    {
        try {
            ...
            $response = Http::timeout(15)->get(config('newsroomflow.sources.gnews.endpoint'), $params);
    ...
    private function fromNewsData(string $topic, string $key): array
    {
        try {
            ...
            $response = Http::timeout(15)->get(config('newsroomflow.sources.newsdata.endpoint'), $params);
    ...
    */
    ...
    {
        if (! config('newsroomflow.llm.enabled') || empty($candidates)) {
            return $candidates;
        }

        $apiKey = config('newsroomflow.llm.api_key');
        if (! $apiKey) {
            return $candidates;
        }
        ...
        try {
            ...
            $response = Http::timeout(30)
                ->withHeaders([
                    'x-api-key'         => $apiKey,
                    'anthropic-version' => config('newsroomflow.llm.version', '2023-06-01'),
                    'content-type'      => 'application/json',
                ])
                ->post(config('newsroomflow.llm.endpoint'), [
                    'model'      => config('newsroomflow.llm.model'),
                    'max_tokens' => 1500,
                    'messages'   => [
