...
class LocalSourceDiscovery
{
    ...
    {
        return (bool) config('newsroomflow.discovery.enabled')
            && (bool) config('newsroomflow.llm.api_key');
    }
    ...
    public function discover(Topic $area): array
    {
        ...
        $max = (int) config('newsroomflow.discovery.max_domains', 6);
    ...
    private function askClaude(string $place, Topic $area): ?string
    {
        ...
        try {
            $response = Http::timeout(60)->withHeaders([
                'x-api-key'         => config('newsroomflow.llm.api_key'),
                'anthropic-version' => config('newsroomflow.llm.version', '2023-06-01'),
                'content-type'      => 'application/json',
            ])->post(config('newsroomflow.llm.endpoint'), [
                'model'      => config('newsroomflow.discovery.model', 'claude-sonnet-5'),
                'max_tokens' => 1024,
                'tools'      => [[
                    'type'     => (string) config('newsroomflow.discovery.web_search_tool', 'web_search_20250305'),
                    'name'     => 'web_search',
                    'max_uses' => (int) config('newsroomflow.discovery.max_searches', 5),
                ]],
                'messages' => [
    ...
    {
        if (! config('newsroomflow.discovery.validate_liveness', true)) {
            return $domains;
        }
        ...
        foreach ($domains as $domain) {
            try {
                $response = Http::timeout(8)
                    ->withHeaders(['User-Agent' => 'Mozilla/5.0 (compatible; NewsroomFlowBot/1.0; +https://newsroomflow.app)'])
                    ->get("https://{$domain}/");
