 ...
 *
 *   php artisan newsroomflow:discover-sources                 # every area missing local sources
 *   php artisan newsroomflow:discover-sources --area=42       # one area
 *   php artisan newsroomflow:discover-sources --city="Erwin" --state=TN   # ad-hoc, no area needed
 *   php artisan newsroomflow:discover-sources --reverify      # re-check learned records past their TTL
 */
class DiscoverLocalSources extends Command
{
    protected $signature = 'newsroomflow:discover-sources
    ...
    public function handle(LocalSourceDiscovery $discovery, LocalSources $sources, TopicRefresher $refresher): int
    {
        if (! $discovery->enabled()) {
            $this->warn('Discovery is disabled (set ANTHROPIC_API_KEY and NEWSROOMFLOW_DISCOVERY=true).');
