...
class ArticleSourcingTest extends TestCase
{
    ...
    {
        config()->set('newsroomflow.sources.thenewsapi.key', null);
        config()->set('newsroomflow.sources.newsdata.key', null);
        config()->set('newsroomflow.sources.gnews.key', null);
        config()->set('newsroomflow.sources.newsapi.key', null);
    ...
    {
        config()->set('newsroomflow.sources.google_news.enabled', true);
        config()->set('newsroomflow.signals.hacker_news', false);
...
XML;
    ...
    public function test_merges_same_story_across_sources_preferring_direct_url(): void
    {
        ...
        config()->set('newsroomflow.sources.thenewsapi.key', 'k1');
        config()->set('newsroomflow.sources.gnews.key', 'k2');
        config()->set('newsroomflow.signals.hacker_news', false);
    ...
    {
        config()->set('newsroomflow.sources.thenewsapi.key', 'test-key');
        config()->set('newsroomflow.signals.hacker_news', true);
    ...
    public function test_llm_rewrites_descriptions_when_enabled(): void
    {
        ...
        config()->set('newsroomflow.llm.api_key', 'sk-test');
        config()->set('newsroomflow.signals.hacker_news', false);
