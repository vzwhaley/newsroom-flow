...
class LocalSourceDiscoveryTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        config()->set('newsroomflow.discovery.enabled', true);
        config()->set('newsroomflow.llm.api_key', 'test-key');
        config()->set('newsroomflow.llm.endpoint', 'https://api.anthropic.com/v1/messages');
        config()->set('newsroomflow.discovery.validate_liveness', false);
    }
    ...
    {
        config()->set('newsroomflow.discovery.enabled', false);
        Http::fake();
    ...
    {
        config()->set('newsroomflow.discovery.validate_liveness', true);
    ...
    public function test_scheduled_sweep_queues_only_areas_that_need_discovery(): void
    {
        ...
        $this->artisan('newsroomflow:discover-sources --reverify --queue')->assertSuccessful();
    ...
    public function test_scheduled_sweep_respects_the_limit(): void
    {
        ...
        $this->artisan('newsroomflow:discover-sources --queue --limit=2')->assertSuccessful();
