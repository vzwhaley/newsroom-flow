...
Served locally by Herd at **https://newspaperflow.test**.
...
- **Command:** `php artisan newspaperflow:refresh` (all topics) ·
...
- **Daily digest:** opted-in users get a "Your NewsroomFlow is ready" email
  (`newspaperflow:digest --due`, scheduled 5 min after the refresh).
...
- `free@newspaperflow.test` — Free (2 topics)
- `pro@newspaperflow.test` — Pro Lifetime (unlimited)
...
```
* * * * * cd /path/to/newspaper-flow-web && php artisan schedule:run >> /dev/null 2>&1
```
...
- [ ] **Mail** — Gmail SMTP is wired; confirm with `php artisan newspaperflow:mail-test you@example.com`.
...
- [ ] **Push notifications** (native apps) — optional. **Android/FCM:** set
      ...
      **iOS/APNs:** set `APNS_KEY_ID`, `APNS_TEAM_ID`, `APNS_BUNDLE_ID`,
      ...
      registration + `newspaperflow:push` still work, nothing is delivered).
