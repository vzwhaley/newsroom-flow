...
class LocalSourceDiscoveryTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        config()->set('newspaperflow.discovery.enabled', true);
        config()->set('newspaperflow.llm.api_key', 'test-key');
        config()->set('newspaperflow.llm.endpoint', 'https://api.anthropic.com/v1/messages');
        config()->set('newspaperflow.discovery.validate_liveness', false);
    }
    ...
    {
        config()->set('newspaperflow.discovery.enabled', false);
        Http::fake();
    ...
    {
        config()->set('newspaperflow.discovery.validate_liveness', true);
    ...
    public function test_scheduled_sweep_queues_only_areas_that_need_discovery(): void
    {
        ...
        $this->artisan('newspaperflow:discover-sources --reverify --queue')->assertSuccessful();
    ...
    public function test_scheduled_sweep_respects_the_limit(): void
    {
        ...
        $this->artisan('newspaperflow:discover-sources --queue --limit=2')->assertSuccessful();
