...
class HybridArticleProvider implements ArticleProvider, LocationAwareProvider
{
    ...
    {
        $pool = (int) config('newspaperflow.candidate_pool', 40);
    ...
    */
    ...
    private function gatherFromSources(string $topic, int $pool): array
    {
        ...
        if (config('newspaperflow.sources.google_news.enabled')) {
            $all = array_merge($all, $this->fromGoogleNewsRss($topic, $pool));
        }

        if ($key = config('newspaperflow.sources.thenewsapi.key')) {
            $all = array_merge($all, $this->fromTheNewsApi($topic, $pool, $key));
        }

        if ($key = config('newspaperflow.sources.newsdata.key')) {
            $all = array_merge($all, $this->fromNewsData($topic, $key));
        }

        if ($key = config('newspaperflow.sources.gnews.key')) {
            $all = array_merge($all, $this->fromGNews($topic, $pool, $key));
        }

        if ($key = config('newspaperflow.sources.newsapi.key')) {
    ...
    {
        return (bool) (config('newspaperflow.sources.google_news.enabled')
            || config('newspaperflow.sources.thenewsapi.key')
            || config('newspaperflow.sources.newsdata.key')
            || config('newspaperflow.sources.gnews.key')
            || config('newspaperflow.sources.newsapi.key'));
    }
    ...
    private function fromGoogleNewsRss(string $topic, int $pool): array
    {
        try {
            ...
            $response = Http::timeout(15)
                ...
                ->get(config('newspaperflow.sources.google_news.endpoint'), $params);
    ...
    private function fromTheNewsApi(string $topic, int $pool, string $key): array
    {
        try {
            ...
            $response = Http::timeout(15)->get(config('newspaperflow.sources.thenewsapi.endpoint'), $params);
    ...
    private function fromNewsApi(string $topic, int $pool, string $key): array
    {
        try {
            ...
            $response = Http::timeout(15)->get(config('newspaperflow.sources.newsapi.endpoint'), $params);
    ...
    private function fromGNews(string $topic, int $pool, string $key): array
    {
        try {
            ...
            $response = Http::timeout(15)->get(config('newspaperflow.sources.gnews.endpoint'), $params);
    ...
    private function fromNewsData(string $topic, string $key): array
    {
        try {
            ...
            $response = Http::timeout(15)->get(config('newspaperflow.sources.newsdata.endpoint'), $params);
    ...
    */
    ...
    {
        if (! config('newspaperflow.llm.enabled') || empty($candidates)) {
            return $candidates;
        }

        $apiKey = config('newspaperflow.llm.api_key');
        if (! $apiKey) {
            return $candidates;
        }
        ...
        try {
            ...
            $response = Http::timeout(30)
                ...
                ->post(config('newspaperflow.llm.endpoint'), [
