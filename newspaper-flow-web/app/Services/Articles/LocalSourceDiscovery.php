...
class LocalSourceDiscovery
{
    ...
    {
        return (bool) config('newspaperflow.discovery.enabled')
            && (bool) config('newspaperflow.llm.api_key');
    }
    ...
    public function discover(Topic $area): array
    {
        ...
        $max = (int) config('newspaperflow.discovery.max_domains', 6);
    ...
    private function askClaude(string $place, Topic $area): ?string
    {
        ...
        try {
            $response = Http::timeout(60)->withHeaders([
                ...
                'model'      => config('newspaperflow.discovery.model', 'claude-sonnet-5'),
                'max_tokens' => 1024,
                'tools'      => [[
                    'type'     => (string) config('newspaperflow.discovery.web_search_tool', 'web_search_20250305'),
                    'name'     => 'web_search',
                    'max_uses' => (int) config('newspaperflow.discovery.max_searches', 5),
                ]],
                'messages' => [
    ...
    {
        if (! config('newspaperflow.discovery.validate_liveness', true)) {
            return $domains;
        }
        ...
        foreach ($domains as $domain) {
            try {
                $response = Http::timeout(8)
                    ->withHeaders(['User-Agent' => 'Mozilla/5.0 (compatible; NewsroomFlowBot/1.0; +https://newspaperflow.app)'])
                    ->get("https://{$domain}/");
